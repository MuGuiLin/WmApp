<template>
    <div class="goods">
		<section class="goods-type" ref="goodsType">
			<ul>
				<li v-for="(o, i) in goods" :key="i" class="menu-list border-1px" @click="_goodsTyptBtn(i, $event)" :class="{'activation': i === computeListH1}">
					<span class="name" :class="o.type > 1 ? 'txt-left' : ''">
						<i v-show="o.type > 1" class="icon" :class="classMap[o.type]"></i>{{o.name}}
					</span>	
				</li>
			</ul>
		</section>
		...
		<section class="goods-list" ref="goodsList">
			<ul>
				<li v-for="(o, i) in goods" :key="i" class="list-box">
					<h1 class="title">{{o.name}}</h1>
					<ul>
						<li v-for="(j, i) in o.foods" :key="i" @click="openGoodsInfo(j, $event)" class="foods-box border-1px">
...
<script>
    ...
    import shopacrt from '@/components/Store/Shopacrt';
    ...
    export default {
        ...
        methods: {
            async initData() {
                ...
                this.goods = await getGoods({uid: '001'});
...
<style lang="stylus" rel="stylesheet/stylus">
	...
	.goods
		...
		.goods-type
			...
			.menu-list
				display: table; box-sizing: border-box;  width: 100%; height: 56px; font-size: 12px; line-height: 14px; text-align: center; border-1px(rgba(7, 17, 27, 0.2)); 
				&.activation
					background: white; color: #008000; font-size: 14px; border-2px(#008000); font-weight: bold; 
				.name
					display: table-cell;  vertical-align: middle; padding: 0px 12px;
					&.txt-left
		...
		.goods-list
			flex: 1; height: 100%; text-align: left;
			.list-box
				.title
