	...
	<section class="shopacrt">
		...
		<div class="content-top" @click="toggleList">
			<div class="shopacrt-box">
				<div class="shop-car">
					<i class="iconfont icon-cartfill" :class="{'count-yes' : goodsCount > 0}"></i>
		...
		<div class="balls-box">
			<transition-group tag="ul" v-on:before-enter="beforeEnter" v-on:enter="enter" v-on:after-enter="afterEnter">
				<li v-for="(o, i) in balls" :key="i" class="ball" v-show="o.show">
					<i class="iconfont icon-roundaddfill inner"></i>
				</li>	
			</transition-group>
		</div>
		<transition name="shopacrt-list-move">
			<div class="shopacrt-list" v-show="listShow">
				...
				<div class="list-content" ref="shopacrtList">
					<ul>
						<li v-for="(o, i) in selectGoods" :key="i" class="list barder-1px">
							<span class="name">{{o.name}}</span>
							<span class="price">&yen;{{o.price}}</span>
							<buyctrl :foods="o"></buyctrl>
						</li>
					</ul>
				</div>
			</div>
...
<style lang="stylus" rel="stylesheet/stylus">
	...
	.shopacrt
		...
		.content-bot
			...
			.shopacrt-box
				position: relative; top: -12px; left: -1px; width: 56px; height: 56px; border: 2px solid #00c500; border-radius: 50%; margin: 0px 11px;
		.content-top
			display: flex; position: fixed; left: 0px; bottom: 0px; width: 100%; height: 48px; color: rgba(255, 255, 255, 0.4); border-top: 2px solid #00c500; background-color: #141d27; z-index: 100px;
			.shopacrt-box
				...
				.shop-car
					...
					& > .count-yes
						background-color: #00c500; color: white; font-weight: 0px;
					.goods-count
						display: inline-block; position: absolute; top: 0px; right: 0px; height: 16px; padding: 0px 5.5px; text-align: center; border-radius: 16px; line-height:16px; font-size: 9px; font-weight: 700; color: white; background-color: #00c500; box-shadow: 0px 2px 4px 0px rgb(0, 0, 0, 0.4);
				.total-price
		...
		.balls-box
			.ball
				...
				.inner
					display: inline-block; font-size: 24px; color: #00c500; border-radius: 50%; transition: all 0.4s linear;
		.shopacrt-list
