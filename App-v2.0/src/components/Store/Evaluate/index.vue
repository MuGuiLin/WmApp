<template>
	<section class="evaluate">
		...
		<div @click="toggle($event)" class="switch">
			<i class="iconfont icon-roundcheckfill" :class="{'on': cont}"></i>
			<span>只看有内容的评价</span>
		</div>
	</section>
</template>
...
<style lang="stylus" rel="stylesheet/stylus">
	...
	.evaluate
		.type
			...
			& > .positive
				background-color: rgba(0, 198, 0, 0.2);
				&.active
					color: white; background-color: #00c500;
			& > .neagtive
				background-color: rgba(77, 85, 93, 0.2);
				&.active
					color: white; background-color: rgb(77, 85, 93);
		.switch
			padding: 12px 18px; line-height: 24px;  color: rgb(147, 135, 159); border-bottom: 1px solid rgba(7, 17, 27, 0.2); text-align: left;
			& > i
