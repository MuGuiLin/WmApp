<template>
	<section class="buyctrl">
		<transition name="movetoleft">
			<div class="less-btn icon-remove_circle_outline" v-show="foods.count > 0" @click.stop.prevent="goodsLessFn($event)"></div>
		</transition>
		<div class="cont-num" v-show="foods.count > 0">{{foods.count}}</div>
		<div class="plus-btn icon-add_circle" @click.stop.prevent="goodsPulsFn($event)"></div>
	</section>
</template>
...
<style lang="stylus" rel="stylesheet/stylus">
	.buyctrl
		font-size: 0px;
		.less-btn, .plus-btn
			display: inline-block; padding: 6px; color: #22affe; font-size: 24px; line-height: 24px;
