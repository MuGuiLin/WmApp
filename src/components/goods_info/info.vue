		...
		<section ref="goodsInfoPage" v-show="showGoodsInfo" class="goods-info-page">
			<div class="info">
				<div class="info-header">
					...
					<i @click="hideGoodsInfoFn" class="icon-arrow_lift"></i>
				</div>
				...
				<div class="info-rating">
					<ul v-show="goods.ratings && goods.ratings.length">
						<li v-show="needShwo(i.rateType, i.text)" class="border-1px" v-for="i in goods.ratings">
							<div class="user">
								<span>{{i.username}}</span>
								<img :src="i.avatar" width="18px" height="18px" />
							</div>
							<div class="time">
								{{i.rateTime | formatDate}}
							</div>
							<div class="text">
								<i :class="{'icon-thumb_up': i.rateType === 0, 'icon-thumb_down': i.rateType === 1}"></i>
								<span v-if="i.text">{{i.text}}</span>
								<span v-else>
									<label v-if="i.rateType === 0">这哥门不错，虽然什么也没说，至少点了个赞！</label>
...
<style lang="stylus" rel="stylesheet/stylus">
	.goods-info-page
		...
		.info-header
			...
			& > i
				position: absolute; top: 12px; left: 12px; display: block; padding: 6px; color: gainsboro; text-align: center; border: 1px solid white; border-radius: 50%; box-shadow: 0px 0px 4px 0px gainsboro;
		.info-title
			...
			.buy-btn
				position: absolute; right: 18px; bottom: 18px;  height: 24px; line-height: 24px; padding: 0px 12px; font-size: 12px; color: white; z-index: 10; border-radius: 12px; background-color: rgb(0, 160, 220);	
