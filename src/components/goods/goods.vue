<template>
	<div class="goods">
		<section class="goods-type" ref="goodsType">
			<ul>
				<li v-for="(i, index) in goods" class="menu-list border-1px" @click="_goodsTyptBtn(index, $event)" :class="{'activation': index === computeListH1}">
		...
		<section class="goods-list" ref="goodsList">
			<ul>
				<li v-for="(i, index) in goods" class="list-box">
					<h1 class="title">{{i.name}}</h1>
					<ul>
						<li v-for="(j, index) in i.foods" @click="openGoodsInfo(j, $event)" class="foods-box border-1px">
